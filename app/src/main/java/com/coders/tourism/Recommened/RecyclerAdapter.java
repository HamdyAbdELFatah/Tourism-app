...
public class RecyclerAdapter extends RecyclerView.Adapter<RecyclerAdapter.ViewHolder>{
    ...
    byte f=0;
    public RecyclerAdapter(ArrayList<Items> arr, Context c) {
        this.arr = arr;
        this.c = c;
    }
    ArrayList<Items> arr;
    ...
    public void onBindViewHolder(@NonNull final ViewHolder holder, final int i) {
        ...
        final String name = prefs.getString(arr.get(i).name, "Noname");
        ...
        holder.favorite.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                GradientDrawable Circle = (GradientDrawable) holder.favorite.getBackground();
                SharedPreferences.Editor editor = c.getSharedPreferences("FAVORITE", c.MODE_PRIVATE).edit();
                if (f == 0) {
                    Circle.setColor(c.getResources().getColor(R.color.colorAccent));
                    holder.favorite.setImageDrawable(c.getResources().getDrawable(R.drawable.favorite));
                    editor.putString(arr.get(i).name, "true");
                    editor.apply();
                    f = 1;
                } else {
